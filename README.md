...
사이드바 **최근 소식지**(`/admin/newsletters`)에서 작성·수정·공개·숨김·삭제를 관리하고, **국가 관리**(`/admin/countries`)에서 국가 옵션의 추가·이름 수정·순서 변경·삭제를 할 수 있습니다. 게시글이 연결된 국가는 삭제할 수 없습니다. 게시글 삭제는 휴지통 이동이며 숨김 상태로 복원할 수 있습니다.

작성 화면은 제목·요약·서식 있는 본문, 4개 언어, 썸네일, PDF 첨부를 지원합니다. 작성 탭은 영어 → 스페인어 → 포르투갈어 → 한국어 순서이며 새 글은 영어 탭에서 시작합니다. 영어 제목과 공개할 본문을 우선 작성하되 기존 한국어 전용 글도 수정할 수 있습니다. 비어 있는 번역은 영어, 기존 글은 한국어로 대체 표시합니다. 썸네일은 비율을 유지하면서 최대 긴 변 1200px, WebP 300KB로 줄여 저장합니다. 기존 업로드는 다시 저장하지 않는 한 바뀌지 않습니다. 공개한 글만 홈페이지 최근 소식지와 `/newsletter`에 나타납니다. 게시글 주소는 `/newsletter/{id}`입니다. 예전 `/newsletter/write` 주소는 관리자 작성 화면으로 이동합니다. 기존 브라우저 localStorage의 글이나 하드코딩된 예시 글은 실제 게시물로 자동 이전하지 않습니다.
...
사이드바 **문의 유형**(`/admin/contact-types`)에서 유형의 순서·이름·추가·삭제와 네 언어의 표시 이름을 관리합니다. 이 작업은 기존 SMTP 전송 설정을 변경하지 않습니다.
...
사이드바 **유튜브 자동화**(`/admin/youtube`)에 채널 ID, `https://www.youtube.com/channel/UC…` 주소 또는 `@핸들` 주소를 저장합니다. @핸들 확인에는 API 키가 먼저 필요합니다. 서버 환경변수 `YOUTUBE_API_KEY`에는 같은 Google Cloud 프로젝트에서 YouTube Data API v3를 활성화해 발급한 API 키를 넣습니다. API 키는 브라우저나 Git에 넣지 마세요. 서버는 해당 채널의 공개 업로드 목록을 15분간 캐시합니다. 채널 또는 키가 없거나 API가 응답하지 않으면 홈페이지에 준비 중 안내만 표시합니다. 가짜 영상은 표시하지 않습니다.
